...
public class WallFollower extends AbstractSolver {
    ...
    public void drawPrevious(Cell cell) {
        String helper;
        helper = tilegroup.getChildren().get(getIndex(cell.getRow(), cell.getCol())).getStyle();
        helper = helper.replace("#a3c9f8", "#aaa");
        tilegroup.getChildren().get(getIndex(cell.getRow(), cell.getCol())).setStyle(helper);
    }
}